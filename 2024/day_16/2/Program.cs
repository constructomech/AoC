...
int FindShortestCostPath(FixedBoard<char> board, Vec2 start, Vec2 target) {
    ...
    var bestCosts = new Dictionary<(Vec2 pos, Direction dir), (int cost, List<PathNode> paths)>();
    ...
    while (pq.TryPeek(out path, out cost)) {        
        ...
        (int cost, List<PathNode> paths) bestPathsForCell;
        if (bestCosts.ContainsKey((path.Pos, path.Dir))) {
            bestPathsForCell = bestCosts[(path.Pos, path.Dir)];
        } else {
        ...
        if (board[nextPosInDir] == '.' && !path.Contains(nextPosInDir)) {
            ...
            (int cost, List<PathNode> paths) bestCostPaths;
            int bestCost = bestCosts.TryGetValue((nextPosInDir, path.Dir), out bestCostPaths) ? bestCostPaths.cost : bestCost = int.MaxValue;
        ...
        foreach (var turnDir in new Direction[] { TurnRight(path.Dir), TurnLeft(path.Dir) }) {
            ...
            if (board[nextPosInTurnDir] == '.' && !path.Contains(nextPosInTurnDir, turnDir)) {
                ...
                (int cost, List<PathNode> paths) bestCostPaths;
                int bestCost = bestCosts.TryGetValue((path.Pos, turnDir), out bestCostPaths) ? bestCostPaths.cost : bestCost = int.MaxValue;
...
public class PathNode {
    ...
    public IEnumerable<Vec2> GetPathPositions() {
        var current = this;
        while (current != null) {
