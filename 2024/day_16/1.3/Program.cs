...
void Run(string[] input) {
    ...
    var nonHallways = new Dictionary<Vec2, Direction>();
    Vec2 startPos = new(0, 0);
    Vec2 endPos = new(0, 0);
    ...
    board.ForEachCell((pos, c) => {
        if (c != '#') {
            ...
            var connectedness = Direction.None;
            foreach (var dir in new Direction[] { Direction.Left, Direction.Up, Direction.Right, Direction.Down }) {
                var testPos = pos + OffsetFromDirection(dir);
            ...
            var horizontalHallway = (connectedness == (Direction.Left | Direction.Right));
            var verticalHallway = (connectedness == (Direction.Up | Direction.Down));

            if (c == 'S' || c == 'E' || !(horizontalHallway || verticalHallway)) {
                nonHallways.Add(pos, connectedness);
                if (c == 'S') {
                    startPos = pos;
                }
                else if (c == 'E') {
    ...
    var graph = new Graph<(Vec2 pos, Direction robotDir)>();
    graph.AddVertex((startPos, Direction.Right));

    var q = new Queue<(Vec2 pos, Direction robotDir)>();
    q.Enqueue((startPos, Direction.Right));

    while (q.Count > 0) {
        (var pos, var robotDir) = q.Dequeue();
        var connectedness = nonHallways[pos];
        
    }


    // var allVerts = graph.SelectVerticies((_, _) => true);
    // var vertPositions = allVerts.Select(item => item.data.pos).ToHashSet();

    // board.Print(c => c, vertPositions.ToList(), '@');

    // // Don't forget special case to rotate robot from start
    // var q = new Queue<(Vec2 pos, Direction entryDir)>();
    // q.
...
[Flags]
public enum Direction { None = 0, Up = 2, Down = 4, Left = 8, Right = 16 }
...
public class Graph<TData> {
    ...
    public IEnumerable<(TData data, List<(int vertexIndex, int weight)> edges)> SelectVerticies(Func<TData, List<(int vertexIndex, int weight)>, bool> predicate) {
