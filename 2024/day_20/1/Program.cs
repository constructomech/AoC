...
List<Vec2> FindMinPath(FixedBoard<char> board, Vec2 start, Vec2 end, (Vec2 pos, Vec2 dir)? bridge = null) {
    ...
    while (q.Count > 0) {
        ...
        foreach (var dir in CardinalAdjacent) {
            ...
            if (board.IsInBounds(target) && 
                (board[target] != '#' || (bridge != null && dir == bridge.Value.dir && target == bridge.Value.pos)) && 
...
Dictionary<int, int> CountCheatPlacements(FixedBoard<char> board, Vec2 start, Vec2 end, int minWin) {
    ...
    int baselinePathLength = baselinePath.Count;
    baselinePath.Insert(0, start);

    var q = new Queue<(Vec2 pos, Vec2 dir)>();

    // Generate valid cheat placements
    foreach (var pos in baselinePath) {

        foreach (var dir in CardinalAdjacent) {
            var target = pos + dir;

            if (!baselinePath.Contains(target)) {

                if (board[target] == '#' && board.IsInBounds(target + dir) && board[target + dir] == '.') {
                    q.Enqueue((target, dir));
                }
            }
        }
    }
    ...
    while (q.Count > 0) {
        (var pos, var dir) = q.Dequeue();
        
        var cheatPath = FindMinPath(board, start, end, (pos, dir));
