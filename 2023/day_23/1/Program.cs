...
static class Fun {
    ...
    private static bool AllowedDirectionally(char location, Pos offset) {
        if (location == '.') {
            return true;
        }
        else if (offset.x == -1 && location == '<') {
            return true;
        }
        else if (offset.x == 1 && location == '>') {
            return true;
        }
        else if (offset.y == -1 && location == '^') {
            return true;
        }
        else if (offset.y == 1 && location == 'v') {
            return true;
        }
        return false;
    }
            ...
            if (node.Position == end) {
                ...
                foreach (var offset in offsets) {
                    ...
                    if (targetPos.x >= 0 && targetPos.y >= 0 && targetPos.y < data.Length && targetPos.x < data[targetPos.y].Length) {
                        ...
                        if (data[targetPos.y][targetPos.x] != '#' && AllowedDirectionally(data[targetPos.y][targetPos.x], offset)) {
