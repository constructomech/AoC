...
public class Field {
    ...
    public void Compress() {
        ...
        // First sort by min Z
        _bricks.Sort((a, b) => Math.Min(a.brick.Corner1.z, a.brick.Corner2.z).CompareTo(Math.Min(b.brick.Corner1.z, b.brick.Corner2.z)));
        ...
        while (true) {
            ...
            if (IsAtRest(_bricks[i].brick)) {
                atRestIndex++;
            }
            else {
                // Move down as long as no target positions are occupied
                do {
                    _bricks[i] = (MoveBrickDown(_bricks[i].brick), true);
                } while (!IsAtRest(_bricks[i].brick));
                atRestIndex = 0;
            }
        }
    }
    ...
    public IEnumerable<Field> GetFieldsMinusOneBrick() {
        foreach (var (removeBrick, _) in _bricks) {
            var newField = new Field();
            foreach (var (brick, _) in _bricks) {
                if (brick != removeBrick) {
                    newField.addBrick(brick);
                }
            }
    ...
    public int CountMovedBricks() {
        var result = 0;
        foreach (var (_, moved) in _bricks) {
            if (moved) {
                result++;
            }
        }
    ...
    private List<(Brick brick, bool moved)> _bricks = new List<(Brick brick, bool moved)>();
...
static class Fun {
    public static void Run(string[] input) {
        ...
        var result = 0;
        foreach (var altField in field.GetFieldsMinusOneBrick()) {
            altField.Compress();
            result += altField.CountMovedBricks();
        }
