...
var sensorPositions = new List<Point>();
int scannerNumber = -1;
...
sensorPositions.Add(new Point() { X = 0, Y = 0, Z = 0 });
...
for (int i = 0; i < sensorPositions.Count; i++) {
    for (int j = 0; j < sensorPositions.Count; j++) {
        var difference = sensorPositions[i].Subtract(sensorPositions[j]);
        
        int manhattanDistance = Math.Abs(difference.X) + Math.Abs(difference.Y) + Math.Abs(difference.Z);
...
Console.WriteLine("Size of combined: {0}, with max Manhattan distance: {1}", accum.Count, maxManhattanDistance);
        ...
        foreach (var permutation in cloud2.GetPermutations()) {
            foreach (var beacon2 in permutation.Beacons) {
                ...
                if (sumOfBeacons - combined.Count >= 12) {
                    sensorPositions.Add(difference);
