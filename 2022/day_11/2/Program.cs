...
// Calculate modulo as product of all the (prime) divisible by values
BigInteger modulo = monkies[0].DivisibleBy;
for (int i = 1; i < monkies.Count; i++) {
    modulo *= monkies[i].DivisibleBy;
}
...
for (int round = 0; round < 10000; round++) {
    ...
    foreach (var monkey in monkies) {
        ...
        for (int i = 0; i < monkey.Items.Count; i++) {
            ...
            // Update worry
            BigInteger newWorry = monkey.WorryFunction(monkey.Items[i]);
...
public class Monkey {
    ...
    public List<BigInteger> Items { get; private set;}
    ...
    public Func<BigInteger, BigInteger> WorryFunction { get; set; }
}