...
using (StreamReader reader = File.OpenText("input.txt"))
{
    while (!reader.EndOfStream)
    {
        ...
        var left = Packet.Parse(line);

        line = reader.ReadLine()!;
        var right = Packet.Parse(line);
...
{
    int result = pairs[i].left.CompareTo(pairs[i].right);
    if (result < 0)
    {
        Console.WriteLine("{0} correct order.", i + 1);
        accum += (i + 1);
    }
}

Console.WriteLine("Result: {0}", accum);
